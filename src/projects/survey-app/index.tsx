      ...
      <div className="row" style={{ marginBottom: 12 }}>
        {(['edit', 'take', 'stats'] as const).map((m) => (
          <button key={m} type="button" className={`btn sm ${mode === m ? 'accent' : 'ghost'}`} onClick={() => setMode(m)}>
            {m === 'edit' ? '編輯' : m === 'take' ? '填寫' : '統計'}
          </button>
        ))}
        <span className="muted">回覆數 {responses.length}</span>
      </div>
      {mode === 'edit' && (
        <div className="panel stack">
          ...
          <button
            type="button"
            className="btn ghost"
            onClick={() => setQs((xs) => [...xs, { id: uid('q'), text: '新問題？', options: ['是', '否'] }])}
          >
            新增題目
          </button>
          {qs.map((q) => (
            <div key={q.id} className="list-item stack">
              <input className="field" value={q.text} onChange={(e) => setQs((xs) => xs.map((x) => (x.id === q.id ? { ...x, text: e.target.value } : x)))} />
              <input
                className="field"
        ...
        <div className="panel stack">
          ...
          {qs.map((q) => (
            <div key={q.id} className="stack">
              <div className="label">{q.text}</div>
          ...
          <button
            ...
            onClick={() => {
              ...
              setResponses((rs) => [...rs, { id: uid('r'), answers }])
              setAnswers({})
              setMode('stats')
            }}
          >
          ...
          {qs.map((q) => (
            <div key={q.id}>
              ...
              {q.options.map((o) => {
                ...
                return (
                  <div key={o} className="row" style={{ marginTop: 6 }}>
                    <span style={{ width: 100 }}>{o}</span>
                    <div className="progress" style={{ flex: 1 }}>
                      <div style={{ width: `${pct}%`, height: 8, borderRadius: 4, background: '#6366f1' }} />
                    </div>
